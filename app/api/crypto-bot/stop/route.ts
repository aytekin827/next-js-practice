...
export async function POST(request: Request) {
  try {
    ...
    // 외부 봇 서버에 중지 요청 (GET 방식)
    const queryParams = new URLSearchParams({
      coin: selectedCoin || "KRW-BTC"
    });

    const response = await fetch(`${CRYPTO_BOT_SERVER}/api/bot/stop?${queryParams}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        error: errorData.error || `봇 중지 실패: ${response.status}`
    ...
    return NextResponse.json({
      success: true,
      message: data.message || '봇이 성공적으로 중지되었습니다',
      isRunning: false,
